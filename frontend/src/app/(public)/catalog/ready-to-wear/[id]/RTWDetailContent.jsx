...
const STOCK_STATUS = { IN_STOCK: { label: "In Stock", color: 'var(--color-status-success)' }, LOW_STOCK: { label: "Low Stock", color: 'var(--color-status-warning)' }, OUT_OF_STOCK: { label: "Sold Out", color: "#B71C1C" } };

// Suspense boundary — required because useSearchParams() is used inside the component
export default function RTWDetailPage({ initialItem = null }) {
    return (
        <Suspense fallback={<RTWDetailSkeleton />}>
            <RTWDetailContent initialItem={initialItem} />
        </Suspense>
    );
}

function RTWDetailContent({ initialItem }) {
    const { id } = useParams();
            ...
            <div className="page-container py-8 lg:py-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                    ...
                    <div>
                        ...
                        {images.length > 1 && (
                            <div className="flex gap-2 overflow-x-auto no-scrollbar">
                                {images.map((img, i) => (
                                    <button key={img} onClick={() => setSelectedImage(i)} className={`relative w-16 h-20 rounded-lg overflow-hidden shrink-0 border-2 transition-colors ${selectedImage === i ? "border-primary" : "border-transparent"}`}>
                                        <Image src={img} alt={`${item.name} - image ${i + 1}`} fill className="object-cover" />
                                    </button>
                                ))}
                            </div>
                        )}
                    ...
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
                        ...
                        <h1 className="text-3xl font-bold text-foreground mt-2 mb-2">{item.name}</h1>
                        <p className="text-2xl font-bold font-mono-data text-primary mb-4">{formatCurrency(item.price)}</p>
                            ...
                            <div className="flex gap-2 flex-wrap">
                                {sizes.map((size) => (
                                    <Button
                                        ...
                                        className={`px-4 py-2 h-auto rounded-md text-sm font-semibold transition-colors hover:bg-[#E0E0E0] hover:text-muted-foreground ${selectedSize === size
                                            ? "bg-primary text-white hover:bg-primary hover:text-white"
                                            : isOutOfStock
                                                ? "bg-muted text-text-light line-through cursor-not-allowed hover:bg-muted hover:text-text-light"
                        ...
                        <div className="mb-8">
                            ...
                            <div className="flex items-center gap-3">
                                <Button variant="outline" size="icon" onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-10 h-10 rounded-md border-input flex items-center justify-center hover:bg-muted" disabled={quantity <= 1}>
                                    <Minus size={16} />
                                </Button>
                                <span className="text-lg font-semibold font-mono-data w-8 text-center">{quantity}</span>
                                <Button variant="outline" size="icon" onClick={() => setQuantity(quantity + 1)} className="w-10 h-10 rounded-md border-input flex items-center justify-center hover:bg-muted">
                                    <Plus size={16} />
                                </Button>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 mb-8">
                            <Button
                                ...
                                variant="outline"
                                className="flex-1 flex h-auto items-center justify-center gap-2 px-6 py-3 rounded-md border-2 border-primary text-primary font-semibold hover:bg-[#F8E8F0] hover:text-primary transition-colors disabled:opacity-50"
                            >
                                <ShoppingBag size={16} />
                            ...
                            <Button
                                ...
                                disabled={isOutOfStock || addToCart.isPending}
                                className="flex-1 flex h-auto items-center justify-center gap-2 px-6 py-3 rounded-md bg-primary text-white font-semibold hover:bg-primary/90 transition-colors disabled:opacity-50"
                            >
                                <Zap size={16} />
                                Buy Now
                            </Button>
                        ...
                        {item.fabricDetails && (
                            <div className="mb-6">
                                <h3 className="text-sm font-semibold text-foreground mb-2">Fabric Details</h3>
                                <p className="text-sm text-muted-foreground">{item.fabricDetails}</p>
                            </div>
                        )}
                        {item.careInstructions && (
                            <div>
                                <h3 className="text-sm font-semibold text-foreground mb-2">Care Instructions</h3>
                                <p className="text-sm text-muted-foreground">{item.careInstructions}</p>
                            </div>
                        )}
                    </motion.div>
                </div>
