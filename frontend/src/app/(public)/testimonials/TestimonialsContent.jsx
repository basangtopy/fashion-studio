...
export default function TestimonialsPage({initialData = []}) {
    ...
    const {
        ...
        // Seed the first page with server-fetched data so it renders immediately
        initialData: (() => {
            const hasData = (Array.isArray(initialData) && initialData.length > 0) || (initialData?.testimonials?.length > 0);
            if (!hasData) return undefined;
            return {
                pages: [
                    Array.isArray(initialData)
                        ? { testimonials: initialData, pagination: { page: 1, pages: 1 } }
                        : initialData
                ],
                pageParams: [1],
            };
                ...
                <div className="page-container flex items-center justify-between">
                    <div className="flex gap-2 overflow-x-auto no-scrollbar">
                        <Button
                            ...
                            className={`px-4 py-1.5 h-auto rounded-full text-sm font-medium whitespace-nowrap transition-colors hover:bg-[#E0E0E0] hover:text-muted-foreground ${ratingFilter === 0
                        ...
                        {[5, 4, 3, 2, 1].map((r) => (
                            <Button
                                ...
                                className={`px-3 py-1.5 h-auto rounded-full text-sm font-medium flex items-center gap-1 transition-colors hover:bg-[#E0E0E0] hover:text-muted-foreground ${ratingFilter === r
                                    ? "bg-primary text-white hover:bg-primary hover:text-white"
                                    : "bg-muted text-muted-foreground"
                                    }`}
                            >
                    ...
                    <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
                        {filtered.map((testimonial, i) => (
                            <motion.div
                                ...
                                className={`break-inside-avoid relative rounded-xl bg-white border border-border ${testimonial.isFeatured
                                    ? "ring-2 ring-ring/20 p-8 lg:p-10 text-base"
                                    : "p-6 lg:p-8"
                                    }`}
                            >
                                {/* Decorative quote */}
                                <span className={`absolute top-3 right-5 font-serif text-primary/10 leading-none select-none ${testimonial.isFeatured ? "text-8xl" : "text-6xl"}`}>
                                    &ldquo;
                                </span>
                                ...
                                <p className={`text-muted-foreground leading-relaxed mb-5 ${testimonial.isFeatured ? "text-base" : "text-sm"}`}>
                                    {testimonial.review}
                                </p>

                                {/* Optional Attached Garment Photo */}
                                {testimonial.photoUrl && (
                                    <div className="relative w-full h-48 sm:h-56 mb-5 rounded-lg overflow-hidden border border-border bg-muted">
                                        {/* blurred background */}
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <div className="w-8 h-8 rounded-full bg-[#F8E8F0] flex items-center justify-center text-xs font-bold text-primary">
                                        ...
                                        <div>
                                            ...
                                            {testimonial.source === "ADMIN_ADDED" && (
                                                <p className="text-[10px] text-text-light">Verified by Studio</p>
                                            )}
                                        </div>
                                    </div>
                                ...
                                {testimonial.isFeatured && (
                                    <span className="absolute top-3 left-3 text-[10px] px-2 py-0.5 rounded-full bg-[#F8E8F0] text-primary font-semibold">
                                        Featured
                                    </span>
                                )}
                            </motion.div>
                        ...
                        {isFetchingNextPage && (
                            <>
                                <div className="break-inside-avoid relative rounded-xl bg-white border border-border p-6 lg:p-8 animate-pulse">
                                    <div className="flex gap-0.5 mb-4">
                                        {Array.from({ length: 5 }).map((_, j) => (
                                    ...
                                    <div className="h-4 bg-muted rounded w-5/6 mb-5"></div>
                                ...
                                <div className="break-inside-avoid relative rounded-xl bg-white border border-border p-6 lg:p-8 animate-pulse">
                                    ...
                                    <div className="h-4 bg-muted rounded w-[90%] mb-2"></div>
                                    <div className="h-4 bg-muted rounded w-3/4 mb-5"></div>
                                    <div className="flex items-center gap-2 mt-8">
                                        <div className="w-8 h-8 rounded-full bg-muted"></div>
                                        <div className="flex flex-col gap-1">
                ...
                <div className="text-center mt-12">
                    {isAuthenticated ? (
                        <Button
                            ...
                            className="inline-flex h-auto items-center gap-2 px-6 py-3 rounded-md bg-primary text-white font-semibold hover:bg-primary/90 transition-colors"
                        >
                    ...
                    >
                        ...
                        >
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <Button variant="ghost" size="icon" onClick={() => setShowForm(false)} className="w-8 h-8 rounded-md hover:bg-muted">
                            ...
                            <Button
                                ...
                                className="w-full flex h-auto items-center justify-center gap-2 py-3 rounded-md bg-primary text-white font-semibold hover:bg-primary/90 transition-colors disabled:opacity-50"
                            >
                                <Send size={16} />
