...
const ORDER_MODELS = [
    { value: "MODEL_1", label: "Model 1", desc: "Custom Tailoring. Client provides fabric.", icon: Scissors, color: 'var(--color-brand-primary)', bg: "#FFF5F8" },
    { value: "MODEL_2", label: "Model 2", desc: "Custom Tailoring. Studio sources fabric.", icon: ShoppingBag, color: 'var(--color-status-info)', bg: "#EFF6FF" },
    { value: "MODEL_3", label: "Model 3", desc: "Ready-to-Wear selection.", icon: Shirt, color: 'var(--color-status-success)', bg: "#F1F8E9" }
];
...
function NewOrderForm() {
    ...
    return (
        <div className="min-h-screen bg-muted xl:flex xl:flex-row pb-20 xl:pb-0">
            ...
            <div className="flex-1 p-6 lg:p-12 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                <div className="max-w-2xl mx-auto space-y-8">
                    ...
                    <div className="flex items-center justify-between">
                        <Button variant="ghost" className="text-muted-foreground h-9 gap-1.5 -ml-3" onClick={() => router.back()}>
                            <ArrowLeft size={16} /> Back to Orders
                        </Button>
                        <div className="flex items-center gap-1.5">
                            {[1, 2, 3].map((s) => (
                                <div key={s} className={`w-8 h-1.5 rounded-full transition-colors ${step >= s ? "bg-primary" : "bg-[#E0E0E0]"}`} />
                            ))}
                        </div>
                    </div>

                    <div className="space-y-4">
                        <h1 className="text-3xl font-bold text-foreground tracking-tight">Create Order</h1>
                        <p className="text-muted-foreground">Follow the steps below to configure a new order parameter for your client.</p>
                    </div>
                    ...
                    <AnimatePresence mode="wait">
                        <motion.div
                            ...
                            className="bg-white rounded-2xl border border-border shadow-sm p-6 lg:p-8 space-y-8"
                        >
                            {/* ── STEP 1: CLIENT ── */}
                            {step === 1 && (
                                <div className="space-y-6">
                                    <div>
                                        <h3 className="text-xl font-bold text-foreground">1. Select Client Profile</h3>
                                        <p className="text-sm text-muted-foreground mt-1">Search for an existing client to bind this order to.</p>
                                    </div>
                                    {!preselectedClientId ? (
                                        <div>
                                            <Label className="text-xs font-semibold text-text-light uppercase tracking-wider mb-2 block">Client Database</Label>
                                                                ...
                                                                {(clientResults || []).map((c) => (
                                                                    <CommandItem key={c.id} value={c.id} onSelect={(val) => {
                                                                        ...
                                                                        <div className="flex items-center gap-3 w-full p-1">
                                                                            <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center text-primary-foreground text-xs font-bold shrink-0">{c.fullName?.charAt(0)}</div>
                                                                            <div className="flex-1 min-w-0">
                                                                                <p className="text-sm font-medium text-foreground truncate">{c.fullName}</p>
                                                                                <p className="text-xs text-text-light truncate">{c.email}</p>
                                                                            </div>
                                                                        </div>
                                                                    </CommandItem>
                                                                ))}
                                            ...
                                            {selectedClient && (
                                                <div className="mt-4 flex items-center justify-between p-4 rounded-xl border border-primary/20 bg-primary/5">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-bold shrink-0">{selectedClientName.charAt(0)}</div>
                                                        <div>
                                                            <p className="text-xs text-primary uppercase tracking-wider font-semibold mb-0.5">Selected Client</p>
                                                            <span className="text-sm font-medium text-foreground">{selectedClientName}</span>
                                                        </div>
                                                    </div>
                                                    <Button variant="ghost" size="sm" onClick={() => { setSelectedClient(null); setSelectedClientName(""); }} className="h-8 text-xs text-primary hover:bg-transparent">Clear</Button>
                                                </div>
                                            )}
                                        </div>
                                    ) : (
                                        <div className="bg-surface-2 p-5 rounded-xl border border-border flex items-center gap-4">
                                            <div className="w-12 h-12 bg-[#E0E0E0] rounded-full flex items-center justify-center text-muted-foreground font-bold text-xl">
                                                {selectedClientName ? selectedClientName.charAt(0) : <Loader2 size={20} className="animate-spin" />}
                                            </div>
                                            <div>
                                                <p className="text-xs text-text-light uppercase tracking-wider font-semibold mb-0.5">Pre-bound Client</p>
                                                <p className="text-base font-bold text-foreground">{selectedClientName || "Loading profile..."}</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            {step === 2 && (
                                <div className="space-y-6">
                                    <div>
                                        <h3 className="text-xl font-bold text-foreground">2. Order Specifications</h3>
                                        <p className="text-sm text-muted-foreground mt-1">Select the order blueprint and item specifications.</p>
                                    </div>
                                        ...
                                        {ORDER_MODELS.map((opt) => {
                                            ...
                                            return (
                                                <button key={opt.value} type="button" onClick={() => setForm({ ...form, orderType: opt.value })}
                                                    className={`relative flex flex-col items-start gap-4 p-5 rounded-xl border-2 text-left transition-all duration-150 ${isSelected ? "border-primary bg-[#FFF5F8]" : "border-border bg-surface-2 hover:border-primary/30"}`}>
                                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${isSelected ? "bg-primary text-primary-foreground shadow-sm" : "text-text-light bg-popover border border-border"}`} style={!isSelected ? { background: opt.bg } : {}}>
                                                        <Icon size={18} style={!isSelected ? { color: opt.color } : {}} />
                                                    </div>
                                                    <div>
                                                        <p className={`text-sm font-bold mb-1 ${isSelected ? "text-primary" : "text-foreground"}`}>{opt.label}</p>
                                    ...
                                    {(form.orderType === "MODEL_1" || form.orderType === "MODEL_2") && (
                                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="pt-6 border-t border-border space-y-6">
                                            <div className="flex p-1 bg-surface-2 border border-border rounded-xl max-w-sm">
                                                <button onClick={() => setForm({ ...form, styleSource: "catalog" })} className={`flex-1 text-sm font-semibold py-2 rounded-lg transition-colors ${form.styleSource === "catalog" ? "bg-white shadow-sm text-foreground border border-[rgba(0,0,0,0.04)]" : "text-text-light"}`}>Catalog Style</button>
                                                <button onClick={() => setForm({ ...form, styleSource: "custom" })} className={`flex-1 text-sm font-semibold py-2 rounded-lg transition-colors ${form.styleSource === "custom" ? "bg-white shadow-sm text-foreground border border-[rgba(0,0,0,0.04)]" : "text-text-light"}`}>Custom Build</button>
                                            </div>

                                            {form.styleSource === "catalog" ? (
                                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                                    <Label className="text-sm font-semibold text-foreground mb-2 block">Search Catalog</Label>
                                                ...
                                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                                                    <div>
                                                        <Label className="text-sm font-semibold text-foreground mb-2 block">Custom Description</Label>
                                                    ...
                                                    <div>
                                                        <Label className="text-sm font-semibold text-foreground mb-2 block">Reference Images <span className="font-normal text-text-light">(max 5)</span></Label>
                                    ...
                                    {form.orderType === "MODEL_3" && (
                                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="pt-6 border-t border-border space-y-6">
                                            <Label className="text-sm font-bold text-foreground block">Add Items to List</Label>
                                            <div className="flex flex-col gap-4 items-end bg-surface-2 p-5 rounded-xl border border-border">
                                                <div className="flex-1 w-full">
                                                    <Label className="text-xs text-muted-foreground mb-1.5 block font-semibold">Ready-to-Wear</Label>
                                                ...
                                                <div className="flex flex-col md:flex-row items-end w-full gap-4">
                                                    <div className="w-full">
                                                        <Label className="text-xs text-muted-foreground mb-1.5 block font-semibold">Size Variant</Label>
                                                        <CustomSelect
                                                    ...
                                                    <div className="w-full ">
                                                        <Label className="text-xs text-muted-foreground mb-1.5 block font-semibold">Quantity</Label>
                            ...
                            {step === 3 && (
                                <div className="space-y-6">
                                    <div>
                                        <h3 className="text-xl font-bold text-foreground">3. Fulfillment & Extras</h3>
                                        <p className="text-sm text-muted-foreground mt-1">Specify how this order will interact with logistics.</p>
                                    </div>

                                    <div className="space-y-6 border-b border-border pb-6">
                                        <Label className="text-sm font-bold text-foreground block">Fulfillment Method</Label>
                                            ...
                                            {FULFILLMENT_OPTIONS.map((opt) => {
                                                ...
                                                return (
                                                    <button key={opt.value} type="button" onClick={() => setForm({ ...form, fulfillmentMethod: opt.value })}
                                                        className={`flex items-center gap-4 p-4 rounded-xl border-2 text-left transition-all duration-150 ${isSelected ? "border-primary bg-[#FFF5F8]" : "border-input hover:border-primary/30"}`}>
                                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isSelected ? "bg-primary text-primary-foreground shadow-sm" : "text-text-light bg-surface-2"}`}>
                                                            <Icon size={18} />
                                                        </div>
                                                        <span className={`text-base font-semibold ${isSelected ? "text-primary" : "text-foreground"}`}>{opt.label}</span>
                                                    </button>
                                                );
                                            })}
                                        </div>
                                        ...
                                        {form.fulfillmentMethod === "DELIVERY" && (
                                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}>
                                                <Label className="text-sm font-semibold text-foreground mb-2 block">Delivery Address *</Label>
                                    ...
                                    {(form.orderType === "MODEL_1" || form.orderType === "MODEL_2") && (
                                        <div className="space-y-6 pt-2">
                                            ...
                                            <div>
                                                <Label className="text-sm font-semibold text-foreground mb-2 block">Fabric Notes <span className="text-text-light font-normal">(Optional)</span></Label>
                                    ...
                                    <div className="pt-2">
                                        <Label className="text-sm font-semibold text-foreground mb-2 block">Client Notes <span className="text-text-light font-normal">(Optional)</span></Label>
                            ...
                            <div className="flex items-center justify-between pt-6 border-t border-border">
                                {step > 1 ? (
                                    <Button variant="ghost" onClick={() => setStep(step - 1)} className="text-muted-foreground font-semibold h-12 px-6 gap-2 rounded-xl">
                                ...
                                {step < 3 && (
                                    <Button
                                        ...
                                        className="bg-primary text-primary-foreground hover:bg-primary/90 h-12 px-8 gap-2 rounded-xl font-bold shadow-md shadow-[#C2185B]/20"
                                    >
            ...
            <div className="w-full xl:w-[480px] bg-white border-l border-border p-6 lg:p-10 flex flex-col items-center">
                <div className="w-full max-w-sm sticky top-10 space-y-8 flex-1 flex flex-col">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-[#FFF5F8] text-primary rounded-full flex items-center justify-center">
                            <Sparkles size={18} />
                        </div>
                        <div>
                            <h3 className="text-lg font-bold text-foreground">Order Summary</h3>
                            <p className="text-xs text-text-light uppercase tracking-wider font-semibold">Live Preview</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 space-y-6">
                        ...
                        <div className="space-y-2">
                            <p className="text-xs text-text-light uppercase tracking-wider font-bold">Client Account</p>
                            {selectedClient ? (
                                <p className="text-base font-bold text-foreground">{selectedClientName}</p>
                            ) : (
                                <p className="text-sm text-text-light italic">Pending selection...</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <p className="text-xs text-text-light uppercase tracking-wider font-bold">Type Blueprint</p>
                            </div>
                            {form.orderType ? (
                                <div className="bg-surface-2 border border-border rounded-xl p-4 space-y-3">
                                    ...
                                    {(form.orderType === "MODEL_1" || form.orderType === "MODEL_2") && (
                                        <div className="space-y-1.5 pt-2 border-t border-border">
                                            <p className="text-xs text-foreground flex justify-between"><span className="text-muted-foreground">Source:</span> <span className="font-semibold">{form.styleSource === "catalog" ? "Catalog Model" : "Custom Built"}</span></p>
                                            <p className="text-xs text-foreground flex justify-between"><span className="text-muted-foreground">Measurements:</span> <span className="font-semibold">{form.useSavedMeasurements ? "Saved Profile" : "Requires Updating"}</span></p>
                                        </div>
                                    )}
                                    ...
                                    {form.orderType === "MODEL_3" && (
                                        <div className="space-y-2 pt-2 border-t border-border">
                                            {form.model3Items.length === 0 ? (
                                                <p className="text-xs text-text-light italic">No items added yet...</p>
                                                ...
                                                <div key={i} className="flex justify-between items-center bg-white p-2 rounded border border-[rgba(0,0,0,0.04)] shadow-sm">
                                                    <div className="flex items-center gap-2">
                                                        <button type="button" onClick={() => removeRtwItem(i)} className="p-1 hover:text-primary transition-colors"><X size={12} /></button>
                                                        <p className="text-xs text-foreground font-medium">{item.quantity}x {item.name}</p>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-xs font-bold">{formatCurrency(item.price * item.quantity)}</p>
                                                        <p className="text-[9px] text-text-light uppercase">Size {item.selectedSize}</p>
                                                    </div>
                                                </div>
                                            ))}
                                            {form.model3Items.length > 0 && (
                                                <div className="flex justify-between items-center pt-3 mt-1">
                                                    <p className="text-xs font-bold text-foreground">Subtotal</p>
                                                    <p className="text-sm font-bold text-primary">{formatCurrency(estimatedTotal)}</p>
                                                </div>
                                            )}
                                        </div>
                                    )}
                                ...
                                <p className="text-sm text-text-light italic">Pending configuration...</p>
                            )}
                        </div>

                        {/* Fulfillment Summary */}
                        <div className="space-y-2">
                            <p className="text-xs text-text-light uppercase tracking-wider font-bold">Fulfillment Logistics</p>
                            <p className="text-sm font-semibold text-foreground bg-surface-2 p-3 rounded-xl border border-border inline-block">
                    ...
                    <div className="mt-auto pt-8">
                        ...
                        <p className="text-[11px] text-text-light text-center mt-3">Finalizing pushes payload parameters directly to active tracking.</p>
                    </div>
                </div>
            </div>
        </div>
