...
const STATUS_CONFIG = {
    REQUESTED: { icon: Clock, color: 'var(--color-status-warning)', bg: "#FFF3E0", label: "Awaiting confirmation from the studio" },
    CONFIRMED: { icon: CheckCircle, color: 'var(--color-status-success)', bg: "#E8F5E9", label: "Your appointment is confirmed" },
    COMPLETED: { icon: CheckCircle, color: 'var(--color-text-dark)', bg: "#F4F0F8", label: "Appointment completed" },
    CANCELLED: { icon: XCircle, color: 'var(--color-status-error)', bg: "#FFEBEE", label: "This appointment was cancelled" },
};
            ...
            <div className="pb-20 lg:pb-0 max-w-3xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-3">
                    <div>
                        ...
                        <p className="text-sm text-text-light">View and manage your fitting appointments</p>
                    </div>
                    {!hasPendingRequest && (
                        <Button
                            onClick={() => setBookingOpen(true)}
                            className="h-10 px-5 gap-2 bg-primary text-primary-foreground hover:bg-primary/90 transition-all duration-200 hover:-translate-y-0.5"
                        >
                ...
                {isLoading ? (
                    ...
                    <div className="space-y-8">
                        ...
                        {active.length > 0 && (
                            <div>
                                <h2 className="text-xs font-semibold text-text-light uppercase tracking-wider mb-4">Active</h2>
                                <motion.div
                                    initial="hidden"
                                    animate="show"
                        ...
                        {past.length > 0 && (
                            <div>
                                <h2 className="text-xs font-semibold text-text-light uppercase tracking-wider mb-4">Past</h2>
                                <motion.div
                                    initial="hidden"
                                    animate="show"
...
function AppointmentCard({ apt, expanded, onToggle }) {
    ...
    return (
        <motion.div variants={staggerItem} className="rounded-xl border border-border bg-white overflow-hidden hover:border-[rgba(0,0,0,0.12)] transition-all duration-200">
            <div className="p-5">
                ...
                <div className="flex flex-wrap gap-4 text-xs text-muted-foreground mb-4">
                    {apt.requestedDate && (
                        <div className="flex items-center gap-1.5">
                            <CalendarIcon size={12} className="text-text-light" />
                    ...
                    {apt.confirmedDate && (
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-status-success font-medium">Confirmed: {format(new Date(apt.confirmedDate), "MMM d, yyyy")}</span>
                        </div>
                    )}
                </div>
                ...
                {apt.adminNotes && (
                    <div className="p-3 rounded-lg bg-muted text-xs text-muted-foreground mb-3 flex items-start gap-2">
                        <MessageSquare size={12} className="text-primary shrink-0 mt-0.5" />
                        <div>
                            <p className="font-medium text-foreground mb-0.5">Note from the studio</p>
                            <p>{apt.adminNotes}</p>
                        </div>
                    </div>
                )}
                ...
                {apt.cancelReason && (
                    <div className="p-3 rounded-lg bg-[#FFEBEE] text-xs text-destructive mb-3 border border-[#FFCDD2]">
                ...
                {apt.clientNotes && (
                    <button
                        onClick={onToggle}
                        className="flex items-center gap-1 text-xs text-primary font-medium hover:underline transition-colors"
                    >
                ...
                {expanded && apt.clientNotes && (
                    ...
                    >
                        <div className="px-5 pb-5 pt-0">
                            <div className="p-3 rounded-lg bg-surface-2 text-xs text-muted-foreground border border-[rgba(0,0,0,0.04)]">
                                <p className="font-medium text-foreground mb-0.5">Your notes</p>
